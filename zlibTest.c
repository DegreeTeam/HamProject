...
unsigned char send_buffer[1200];
...
void data_sending_handler()
{
	int i;
      	for(i = 0; i < cnt;i++) {
	unsigned int size;
	size = (unsigned int) destLen;
	send(*sockid[i] , &size, 1, 0);
           if ( *(sockid[i]) != -1 && (send(*(sockid[i]), send_buffer, destLen, 0) <= 0)   ) {
...
void *data_streaming(void *socket_desc)
{
 ...
 /* 2 bytes/sample, 2 channels */
  ...
  while (1) {
	...
	compress2(send_buffer, &destLen, (const Bytef*)buffer, srcLen, 1);
